...
- **`odata-rs-routing`** — axum middleware that rewrites OData subsegment key URLs (`/EntitySet('key')`) into segment form (`/EntitySet/__key__/key`) so standard segment-based routers can dispatch them. Also exports `OriginalODataUri` for reconstructing canonical OData URIs in responses, `KEY_SEGMENT` for building rewrite-style route patterns, and `ODataRouterExt` for applying the middleware to a `Router`.
- **`odata-rs-service`** — axum-based router, handler contexts (`CollectionContext`, `EntityContext`, contained variants), and the builder API. Internally projects the EDM model into a router-only working set (`schema_view`, crate-private) built from `csdl_edm::edm::Model`. The public builder accepts a `&csdl_edm::edm::Model` via `ODataServiceBuilder::new`, or accepts raw CSDL via the `from_csdl` convenience constructor (parse → resolve → project, one call). Re-exports `csdl_edm::{Error, Result}` for convenience. Optionally exposes `odata_service::oquery` (sqlx/SQLite query builder) behind the `sqlx-sqlite` feature. Routes are dual-registered in both segment-style (`/EntitySet/{id}`) and rewrite-style (`/EntitySet/__key__/{id}`) forms; the rewrite middleware from `odata-rs-routing` is applied at build time.

The umbrella crate `odata-rs` re-exports the four under matching features (`edm`, `url`, `routing`, `service`, `sqlx-sqlite`, `full`). The `edm` feature re-exports `csdl_edm` as `odata_rs::edm`.
