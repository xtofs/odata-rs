...
impl<R: BufRead> CsdlReader<R> {
        ...
        match name {
            ...
            "If" => {
                ...
                let else_ = if c.len() == 3 {
                    Some(Box::new(c.remove(2)))
                } else {
                    None
                };
                let then_ = Box::new(c.remove(1));
                let test = Box::new(c.remove(0));
            ...
            "Record" => {
                ...
                let properties = self.read_property_values(name)?;
                Ok(CsdlAnnotationExpression::Record { type_, properties })
            }
    ...
    fn read_property_values(&mut self, parent: &str) -> Result<Vec<PropertyValue>> {
        let mut out = Vec::new();
        loop {
            self.buf.clear();
            match self.inner.read_event_into(&mut self.buf)? {
                Event::Start(e) => {
                    ...
                    if n != "PropertyValue" {
                        return Err(self.csdl_err(format!(
                            "unexpected <{n}> in <{parent}> (expected PropertyValue)"
                        )));
                    }
                    ...
                    let value = pick_first_value(inline, body);
